...
public class Detect2Activity extends com.gwm.base.BaseActivity {
    ...
    private static final int YINCANGINFO = 6635;
...
//                    HwitManager.HwitSetIOValue(5, 0);
    ...
    private long UPDATE_TIME = 1000*60*5;
    ...
    private InitListener mTtsInitListener = new InitListener() {
        ...
        public void onInit(int code) {
            ...
            if (code == ErrorCode.SUCCESS){
                showToast("语音模块初始化成功");
                setParam();
                FlowerCollector.onEvent(Detect2Activity.this, "tts_play");
//                startVoice("欢迎光临");
            }else{
                showToast("语音模块初始化失败");
            }
    ...
    private void start() {
        try {
            ...
            faceDetectManager.start();
            MyLogger.kLog().w("启动成功");
        } catch (Exception e) {
            e.printStackTrace();
            MyLogger.kLog().w("启动失败");
        }
    }
    ...
    private void initView() {
        ...
        cameraImageSource.getCameraControl().setPermissionCallback(new PermissionCallback() {
            ...
            public boolean onRequestPermission() {
                ActivityCompat.requestPermissions(Detect2Activity.this,
...
//        cameraImageSource.getCameraControl().setCameraFacing(ICameraControl.CAMERA_USB);
    ...
    private void initBrightness() {
        int brightness = BrightnessTools.getScreenBrightness(Detect2Activity.this);
        if (brightness < 200) {
...
//                tv.setText(s.toString());
    ...
    public void faceimages(String path) {
        android.util.Log.w("cmd","上传图片--------------");
        //TODO 检测活体，1.获取TOKEN   2.活体检测
//        BaiduHTDataPresenter baiduHTDataPresenter = new BaiduHTDataPresenter(this,this);
//        baiduHTDataPresenter.ht();
        ...
        if (data instanceof FaceImgBean){
            ...
            if (bean.code == 200) {
                ...
                if (bean1 == null || bean1.data.name == null) {
                    ...
                    if (bean1.data.name.equals(bean.data.name) && currentTime < 1000*5){  //判断1分钟内是同一人  不做欢迎处理  只是更新当地的保存人脸数据
                        aCache.put("renData", bean);
                        aCache.put("renDataTime",System.currentTimeMillis());
                        return;
                    }else {
...
//                    startVoice(bean.data.name+",您好!请前往5楼商务部进行接待,谢谢!");
                ...
                rl_shenfen.setVisibility(View.VISIBLE);
            }
...
//			*/
    ...
    @OnClick({R.id.iv_shangwu,R.id.iv_mianshi})
    public void onClick(View view){
        handler.removeCallbacks(runnable);
        Intent intent = new Intent(Detect2Activity.this,RegistActivity.class);
        switch (view.getId()){
            case R.id.iv_shangwu:
                intent.putExtra(RegistActivity.TYPE,2);
                startActivity(intent);
                finish();
                break;
            case R.id.iv_mianshi:
                intent.putExtra(RegistActivity.TYPE,2);
                startActivity(intent);
                finish();
                break;
