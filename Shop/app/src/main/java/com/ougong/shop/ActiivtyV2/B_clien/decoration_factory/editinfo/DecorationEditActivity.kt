package com.ougong.shop.ActiivtyV2.B_clien.decoration_factory.editinfo
...
class DecorationEditActivity : BaseActivity<EditInfoContract.View, EditInfoContract.Presenter>(), EditInfoContract.View,
    OnDateSetListener {
    ...
    override fun setContentViewSource() = R.layout.activity_decoration
...
//    override fun initPresenter() = EditInfoPresenter()
    ...
    fun selectPicFromLocal() {
        ...
        startActivityForResult(intent, ConstString.REQUEST_CODE_LOCAL)
    }
...
//            }
            ...
            if (TextUtils.isEmpty(name_mail_address.text.toString())) {
                ToastUtils.toast(this, "邮箱不能为空")
                return@setOnClickListener
            }
            mUser.mail_address = name_mail_address.text.toString()

            if (TextUtils.isEmpty(name_fox_no.text.toString())) {
                ToastUtils.toast(this, "传真不能为空")
                return@setOnClickListener
            }
            mUser.fox_no = name_fox_no.text.toString()


            if (TextUtils.isEmpty(name_phone_no.text.toString())) {
                ToastUtils.toast(this, "固定电话不能为空")
                return@setOnClickListener
            }
            mUser.home_phone = name_phone_no.text.toString()

            if (TextUtils.isEmpty(name_factory_categary.text.toString())) {
                ToastUtils.toast(this, "公司行业不能为空")
                return@setOnClickListener
            }
            mUser.factory_cateary = name_factory_categary.text.toString()


            if (TextUtils.isEmpty(name_video_address.text.toString())) {
                ToastUtils.toast(this, "视频地址不能为空")
                return@setOnClickListener
            }
            mUser.vido_url = name_video_address.text.toString()


            if (TextUtils.isEmpty(name_company_describe.text.toString())) {
                ToastUtils.toast(this, "公司行业不能为空")
                return@setOnClickListener
            }
            mUser.factory_describe = name_company_describe.text.toString()
...
//        presenter.uploadImage("")
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            ...
            ConstString.REQUEST_CODE_LOCAL -> {
